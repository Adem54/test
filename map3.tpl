...
<script>
	...
	console.log("class_mapLayer: ",class_mapLayer);
	let result = class_mapLayer.modifyMapLayersByFeatureFormat(aMapLayers);
		...
		$.ajax({
			...
			success: function (sJsonRet) {
				...
				if (oRet.bRet)
				{
					...
					//iste burda aObjectList data si aslinda alinmis ve classMap.aFlags a atanarak hemen alt satirda createFlagLayer a argumen olarak verilip bu method invoke ediliyor ve de bu da map tek i  områdeler icerisinde var olan feature datasini olusturuyor...ve bu sayede bu datalari gorebiliyoruz 
					//629 adet objectlerden olusan bir arraydir ve icerisinde 
/*
			continues: "F"	
		garbru: "0940-62/175"	
		logstatus: "FINI"	
		...
		object_id: 618	
		object_type_id: 1	
		objname: "Furestøylvegen 121"	
		orderidlist: []	
		orderlist: Array(1)	
			0: {postcode: '4748', postcity: 'RYSSTAD', firstname: '', middlename: '', lastname: 'Byggmester Ege', …}
			length: 1
			[[Prototype]]: Array(0)
		paid: "F"	
		position: "POINT(7.435358841271643 59.09980776666927)"	
		servicename: "Brøyte pr. sesong"	
		servicetag: "plowseason"	
		status: "ring"	
		timefinished: "16.02.2018 18:20"	
		validcont: "F"	
		valuetype: "parking"	
	*/
	//Ayrica da sunu farkettim.. bu objname sudur her bir hytte ya biz object diyoruz ve her bir hyttename i search arama kutusunda hytteadresi yani bu bu listedeki objname e karsilik geliyor bu girilerek arama yapildiginda hangi hytte ye karsilk geliyor ise o hytte uzerine map-marker-icon.svg yani bu kirmizi lokasyon iconu geliyor ve direk aranan hytte yi leverandøre gostermis oluyor
	...
	//classMap.defaultStyle map teki icerisinde bayraklar vs olan alanlarin genel olarak style idir
	...
	{{@end-loop:servicearealist}}
		...
		//Burasi cok onemli, map olusturulma islemi ve map hangi layer lardan olusuyor ve hangi feature lar kullanilmis burda bunu burda gorebiliyoruz
		...
		function classMap.clusterStyle(clusterFeature){

			const size = clusterFeature.get('features').length;
			let originalFeature = clusterFeature.get("features")[0];//Burasi her bir feature i temsil ediyor
		...
		// Where am i...Buna bakalim ne icin kullanildigiina
		...
		//Zoom seviyesine gore flag larin da size lari dinamik olarak degistirilmis burda...
...
<script>
	...
	//Bu da haftanin gunlerinin uzerine tiklaninca o gunlere ait yapilan hizmetin dinamik olarak geldigi tum aObjectList yani icinde bulundugmuz leverandørun yani providerid ye ait tum hyttelleri aldigi hizmetlerde icinde getiren ve tiklanan gun butonuna gore getiren bir action islemidir ve bu islemde asagida 2 tane oldugnu goruruz yani 2 ayari buton icinde kullanildigini goruruz bunun 1 tanesi leverandør admin icindir ve leverandørun adminpanel den tiklayip da map e geldiginde karsisina gelen gun-day butonlari asagidaki admin icin olan Onload dur , digeri ise plower icin olan onLoad ise brøytebilsoforlerinin admin-panel deki map e girereken brøyte yaparken girdikleri yerden tiklayarak gorebilmeleri icin yapilmis olabilir.. Ve burda aslinda aObjectList datasi yani classMap.aFlags = oRet.aData.data; bu data tiklanan gune gore guncellenmis olarak område ler icerisine tekrar basilmis oluyor
...
//parametreye buyuklugunu ve src sini veriyoruz yani hangi iconu vs kullanacagini ve style olsuturuan bir method var burda
	...
	//flagLayer in olusturuldugu fonksiyon burasi
	//aObjectList burda parametreye veriliyor ve createFlagLayer invoke edildiginde arguman olarak verilen array datasi ne ise iste o ObjectListtir yani classMap.aFlags datasidir ve bu data da yine ajax_update_flags.php den get all flags isleminden api den getirilen bir datadir..360.satirda.. tum bu datalar ilk defa 386.satirda data ajaxtan gelir gelmez classMap.aFlags=oRet.aData.data ve  createFlagLayer(classMap.aFlags) yapilarak område icerisindeki tum gordgumuz point datalari olusturulmus oluyor..Ve burasi dinamik birsekilde oldugu icin once temizler sonra ise icerisine yaziyor
	function createFlagLayer(aObjectList)
	{
		...
		//aObjectList for-loop ile dondurulerek ardindan icerde orderlist property si de bir dizi o da icerde dondurulerek iconFeature olusturulyor ve icerisiine her bir kullanicinin bilgileri girilerek bunlar overlay-popup da kullaniliyor
			...
			for (let i = 0; i < aObjectList.length; i++)
			{
					...
					case 'ring-extra': // No order, extra.
						...
						for (let ii = 0; ii < aObjectList[i].orderlist.length; ii++)
						{
							...
							if (aObjectList[i].orderlist[ii]['arrivaltime'])
								...
								//Burda aObjectList for-loop ile dondurulurken icerde aOrderList dizisine her bir obje yani hytte- icin oderbilgisinin datasi veriliyor ki hangi hytte ne hizmeti aliiyor vs burdan yonetiliyor galiba
							aOrderList.push({
						...
						//Iste burdada iconFeature olusturulyor ve her bir hytte-point noktasi ve ile birlikte feature icerisine her turlu o hytte ya ait data da atiliyor ki ozellikle overLay-popup ile biz bu datayi uzerine tiklaninca kolay bir sekilde basabilmis oluyoruz... 
					...
					case 'ring-extra': // No order, extra ploving.
						{
							...
							{
							//g_bColorBlind muhtemelen renk korleri icin olabilir
					...
					case 'ring-multi':
						{
							...
							//aObjectList te validcont aktif ise  yani muhtemelen bu kullanici su an bizim sistemmimiz kullanan bizim ile validcontrati olan birisi ise...
							//
				...
				if (iconStyle && iconFeature)
				{
					{{@if:is_snowplow}}
					let aStyleList = [];
					//iconStyle yukarda dinamik bir sekilde..son kullanicilarini kontraktavtale ve, aldiklari hizmetin durumlarina gore dinamik olarak degisen bir style dir..Map te bulunan område icerisindeki flag,kucuk yesil nokta vs gibi tum style larin bir anlami vardir.. ve de ve bu iconStyle tum styleList dizisinin icerisindeki bir elemandir
					...
					if (g_bArrivalTime)
					{
						...
						switch (aObjectList[i]['status'])
						{
							...
							case 'red':
								...
								{
								//Bu da arrivaltime-muhtemelen brøytebil in varis saati veya gun perioduna gore bir text style ayarliyor yani 1,2,3,4,A text lerini feature uzerinde gosterecek olan style i ayarliyor ve bunlarin hepsinin birer anlami var.. 
					...
					if (g_bColorBlind)
					{
						...
						if (ch.length > 0)
						{
						//colorblind ile text style da ayarlanmis yani yine renkkorlerine yonelik boyle bir sey yapilmis muhtemelen
					...
					switch (aObjectList[i]['status'])
					{
						case 'ring-multi':
							...
							{
							//Coklu park islevi nin de kendine ait bir style i olusturulmus biz map ten bu style i gorunce bunun coklu park anlamina geldigni anlayabiliriz
					...
					iconFeature.setStyle(aStyleList); // Set the style.
					//aStyleList aslinda tum style lar yani område lerde gordgumuz tum flag,icon,text vs hepsi aStyleList icinde toplanmis ve bu iconFeature denen feature ye uygulaniyor
					//Su an burasi bir aObjectList yani tum hytte listesinin for-loop ile dongu icerisi ve her bir hytte islem goruyor burda ve yukarda her bir hytte icin feature ousturulduktan sonra iconFeature e atanarak(tabi tum hytte bilgileri de feature icerisine atanmis durumdadir ki overlay-popup da kullanilsin) ve burda da dinamik olarak hazirlanan style lardan olusan(her bir flag,kucuk nokta, renk,text vs bir anlama gelir) aStyleList burda uygulaniyor...ve hytte nin aldigi hizmettir brøytebilin o hytteye varis saati ve bunun gibi bircok kritere gore style i dinamik bir sekilde ayarlaniyor ve her bir hytte yani Object burda bu dinamik style i kullanmis oluyor

					//iconFeature.setStyle(aStyleList); // Set the style.
					{{@end-if:is_snowplow}}

					//Ve de iste tam da burda biz iconFeatureList dizisinin classMap.iconFeatureList dizisi tum feature lerin icinde barindirildigi ama tabi bu områdeler icinde bulunan tum feature larin icinde barindirilidigi ve bizim de ihtiyacimiz olan featureListtir..yani pointlerden olusan ve områdeler icerisinde gordugmz tum, pointFeature lardir bunlar... 
					// Update the list.
					classMap.iconFeatureList.push(iconFeature);
					//BURASI FOR DONGUSUNUN ICI HALA...
				}
			}
			//BURASI FOR DONGUSUNUN DISI
			//Iste områdeler icerisindeki tum point-geometry feature larini feature olarak ekleyen yer tam da burasidir...cok onemli..Ve burdan da zaten sunu goruyoruz områdeler zaten belli. hytte pointleri de belli hepsi bir dizi icerisinde geliyor yani ayri ayri område ye gore ayrilma vs yok ki zaten buna gerek yok...belki her bir hytte icerisidne hangi områdeye ait oldugunu bildiren data bile olabilir.
			//Cluster olustururken kullanacagimiz source iste classMap.vectorFlagSource dir...
			// Pull all the flags onto the map.
			classMap.vectorFlagSource.addFeatures(classMap.iconFeatureList);
			//clusterForVectorFlagSource--CLUSTER OLUSTURMAYI BURDA YAPARIZ ONCELIKLE.. SONRA BU CLUSTER SOURCE YI GIDIP VECTOR-LAYER ICINDE SOURCE OLARAK KULLANIRIZ...VE BU VECTOR LAYER  A DA FUNCTION STYLE I IMPLEMENT EDERIZ KI PARAMETRE YE CLUSTER FEATURE ALACAKTIR.. YANI GRUP FEATURE LERI VEREN BIR FEATURE YANI ICINDE DEFAULT OALRAK FEATURE.GET("FEATURES")[0] DEDIGMIZ ZAMAN
			classMap.clusterForVectorFlagSource = new ol.source.Cluster({
				distance:30,
				minDistance:parseInt(20,10),
				source:classMap.vectorFlagSource
			});
			//Boyle daha mi pratik olur acaba.. diger layer icerisinde normalde classMap.clusterForVectorFlagSource bunun source olrak kullanilmasi gerekiyor cluster i n uygulanmasi icin ama tabi bu source uzerinde farkli islemlerde yapiliyor o islemlerde biz cluster a donusturdukten sonraki interaktif olarak source nin guncellenme islemlerinde bazi seyleri bozabiliriz ondan dolayi da aslinda ayri tutsak daha mantikli olabilir gibi
		/*	classMap.vectorFlagSource = new ol.source.Cluster({
				distance:30,
				minDistance:parseInt(20,10),
				source:classMap.vectorFlagSource
			});  */
			//Yapilacak islem su clustersource ayrica da gidilip layer icinde source olarak kullanilirken orda bir de style kullanilacak fonksiyon olarak ve o fonksiyon icerisinde if-else veye switch case ile cluster gruplari sayi olarak 1 e dustugunde burda dinamik olarak hazirlanan method lar girilecek... yani biz hytte sayilarinin durumuna gore cluster sayilarinin durumuna gore bir style ayarlamasi yapabiliyoruz burdaki kritik nokta ne zaman ki artik her bir hytte gozukur hale gelince yani bu su demek cluster 1 oldugu zaman iste o zaman bu map.tpl icinde hazirlanan dinamik style kendi style iini kullancak iste biz bunu uygulamamiz gerekiyor.... 
	...
	//Burda da zannedersem kullanicinin interaktif bir sekilde kendisine bir pozisyon olusturabilmesi icin artik bu park yeri pozisiyonu da olabilir ama yada muhtemelen bu brøytebil soforu icin yapilmistir brøyte bil isi bitince vs o anda hemen map uzerinde nerde oldugunu gosteren bir pozisyon olusturuyor da olabilir tam emin degilim sonra bir daha bakariz buna... 
	...
	//Burasi område leri olustugu yer.. asagaida da polygon lardan olusan vectorSource implemente ediliyor
