...
<script>
//BU COZUM DE DE VECTORFLAGLAYER VE CLUSTERFLAGLAYER I AYRI AYRI OLSUTURUYORUZ VE BUNLARI CREATEFLAGLAYER METHODU NUN DISINDA VE 380 LI SATIRLARDA CREATEFLAGLAYER IN AJAX-SUCCESS BOLUMUNDE GELEEN DATA YI PARAMETRE OLARAK ALMASI ILE CALISTIRDIGMIZ YERIN HEMEN ALTINDA, MAP.VIEW IN CHANGE-RESOLUTION ILE ZOOM-LEVEL DEGISMESINE GORE ADDLAYER-REMOVELAYER ILE 16 USTU VECTORFLAGLAYER 16 ALTINDA ISE CLUSTERFLAGLAYER I MAP E EKLE DIYORUZ AMA BU DURUMDA DA BIZ DAHA ONCEDEN BU LOGIC LER ICERIISINDE YAPILMIS OLAN BIR DEGISIKLIK VE O DEGISIKLIGIN HEMEN SONRASINDA BIR KEZ DAHA CREATAFLAGLAYER IN INVOKE EDILMESI YANI ASLINDA GUNCELLENMESINI YORUM SATIRINA ALDIK VE PERFORMANS OLARAK EN AZINDAN BIR SEVIYE DAHA IYILESTI AMA BIZ YORUM SATIRINA ALDGIMIZ ISLEMI CREATEFLAGLAYER YAPMADAN YAPABILIRMIYIZ BUNU ARASTIRMALIYZ VE YAPABILIRSEK CREATEFLAGLAYER I HER ZOOM-LEVEL DEGISTIGINDE INVOKE ETMEDEN COZMEYI DE ARASTIRMALIYIZ
					...
					/*
						...
						classMap.map.getView().on('change:resolution', function (evt)
							...
							console.log("Zoom-levellll: ",classMap.map.getView().getZoom());
								...
								{
								console.log("classMap.iZoom >= 18");

									//classMap.iFlagSize = classMap.iLargeSize;
									//createFlagLayer(classMap.aFlags);
									classMap.map.removeLayer(classMap.vectorFlagLayer);
									classMap.map.removeLayer(classMap.clusterFlagLayer);
									classMap.map.addLayer(classMap.vectorFlagLayer);
...
//	let aStyleList = [];
	...
	console.log("classMap.map-start: ",classMap.map.getLayers());
		...
		try
		{
			...
			for (let i = 0; i < aObjectList.length; i++)
			{
				...
				//	 aStyleList = [];
					...
					//iconFeature.setStyle(undefined);
					console.log("iconFeature.getStyle(): ",iconFeature.getStyle());
					classMap.iconFeatureListWithoutStyle.push(iconFeature);
		...
		//	classMap.clusterVectorFlagSource.addFeatures(classMap.iconFeatureListWithoutStyle);
			classMap.vectorFlagSource.getFeatures().forEach(feature=>{
				//console.log("vectorFlagSource; ", feature.getStyle());
			})
			
			classMap.clusterVectorFlagSource.getFeatures().forEach(feature=>{
				//console.log("clusterVectorFlagSource; ", feature.getStyle());
				//feature.setStyle(undefined);//Bu sekilde de bos yuvarlak pointler geliyor yine ayrica cluster in style i haric bunun style i da geliyor yine ondan doolayi biz style siz feature listesi olusturacagiz sadece cluster a ozel ...
		...
		/*	function arrangeClusterByZoomLevel(){
				...
				let source = null;
				if(classMap.map.getView().getZoom() < 18)
				{
	...
	//Burda yapamazsin cunku bu map olusumu tamamen asenkrondur burda map i alamayiz cunku map i olusturan datalar biraz gec geliyor...bizim createFlagLayer methodu icinde calisma zorunlulugumuz var... ok.. 
//	console.log("classMap.map-outsideofCreateFlagLayer: ",classMap.map.getLayers());
